				...
				if (pluginBindings !== undefined) {
					for (const binding of pluginBindings) {
						...
						if ("service" in binding) {
							...
							const userServicePrefix = getUserServiceName("");
							const targetWorkerName = service?.name?.startsWith(
								userServicePrefix
							)
								? service.name.substring(userServicePrefix.length)
								: undefined;
							...
							if (
								...
								service.name = getIngressServiceName(targetWorkerName);
							}
						}
					}
				}
	...
	async #registerWorkers(): Promise<void> {
		...
		for (const workerOpts of this.#workerOpts) {
			...
			entries.push([
				...
				{
					debugPortAddress,
					defaultEntrypointService: getIngressServiceName(workerOpts.core.name),
